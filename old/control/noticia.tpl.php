...
function activar(idNoticia){
	PFHTML.rellenarDato('formBuscar','accion','activar');
	PFHTML.rellenarDato('formBuscar','id_noticia',idNoticia);
	document.getElementById('formBuscar').submit();
}

function desactivar(idNoticia){
	PFHTML.rellenarDato('formBuscar','accion','desactivar');
	PFHTML.rellenarDato('formBuscar','id_noticia',idNoticia);
	document.getElementById('formBuscar').submit();
}
...
<table class="tabla" width="600" border="0" cellspacing="0" cellpadding="0" style="margin-left:20px;" id="set">
  ...
  <tr>
	...
	<td width="25">&nbsp;</td>
	<td width="25"><? echo ($item['visible']==1)? "<a href='#' onclick='desactivar(".$item['id_noticia'].")'><img src='img/icon_activa.gif' width='16' height='16' border='0' /></a>" : "<a href='#' onclick='activar(".$item['id_noticia'].")'><img src='img/icon_desactiva.gif' width='16' height='16' border='0' /></a>"; ?></td>
    ...
    <div style="clear:both;"></div>
     ...
     <div id="contenedorImg">
    	 <? 
	 	 ...
	 	 ?>
   		<div class="container">
	      <a style="cursor:pointer;" onclick="PFHTML.openWindow('<? echo $this->modulo; ?>?<? $this->encodeURL('accion=verVideo&idVideo='.$item['id_video']); ?>','Video',600,450);" target="_blank" ><span class="textoForm">Ver Video</span></a>
    	     <ul>
        		 <li>&nbsp;</li>
				<br>
