...
function subirPos(idProd,idCat){
	PFHTML.rellenarDato('formBuscar','accion','subirPos');
	PFHTML.rellenarDato('formBuscar','id_producto',idProd);
	PFHTML.rellenarDato('formBuscar','id_categoria',idCat);
	document.getElementById('formBuscar').submit();
}

function bajarPos(idProd,idCat){
...
$(document).ready(function(){
	id=$("#id_categoria_vieja").attr("value");
	//alert(id);
	if(id==0){
...
<table class="tabla" width="600" border="0" cellspacing="0" cellpadding="3" style="margin-left:20px;" id="set">
  <tr class="encabezadoTabla">
    <td colspan="5" >
        <form id="formBuscar" name="formBuscar" method="post" action="<? echo $this->modulo; ?>">
	    	...
	    	<input type="hidden" name="accion" id="accion" value="listar" />
				...
				<input type="hidden" name="id_categoria_vieja" id="id_categoria_vieja"/>
    ...
    <td colspan="5" align="center" ><a href="<? echo $this->modulo; ?>?<?  $this->encodeURL('accion=nuevo'); ?>" class="tituloListado">Nuevo Producto</a></td>
    </tr>
  ...
  <tr>
	...
	<td width="25"><? echo ($item['activo']==1)? "<img src='img/icon_activa.gif' width='16' height='16' border='0' />" : "<img src='img/icon_desactiva.gif' width='16' height='16' border='0' />"; ?></td>
    <td width="25" align="center" valign="middle" class="textoListado"><span class="subir" style="display:none"><a href="#" onclick="subirPos(<? echo $item['id_producto']?>,<? echo $item['id_categoria']?>)"><img src="img/arriba.gif" width="16" height="16" border="0" /></a></span></td>
    <td width="25" align="center" valign="middle" class="textoListado"><span class="bajar" style="display:none"><a href="#" onclick="bajarPos(<? echo $item['id_producto']?>,<? echo $item['id_categoria']?>)"><img src="img/abajo.gif" alt="" width="16" height="16" border="0" /></a></span></td>
    ...
    <td width="25" align="center" valign="middle" class="menu_categorias"><a href="<? echo $this->modulo; ?>?<? $this->encodeURL('accion=eliminar&id_producto='.$item['id_producto'].'&id_categoria='.$item['id_categoria'] ); ?>" onclick="return objConfirmacion.requerirConfirmacion(this,'Está seguro de eliminar el prod <? echo $item['titulo']; ?>?');"><img src="img/delete.gif" width="16" height="16" border="0" /></a></td>
  </tr>
  
  	<? } ?>
  <tr>
    <td colspan="10"><? if($this->oPaginador->paginaAnterior!==false){ ?>
	...
	<div>
			...
			<div >
		    	<label class="inputText" >Descipcion de Promoci&oacute;n:</label>
	    			<input name="descripcion_promocion" type="text" class="txtInput" id="descripcion_promocion"  value="" maxlength="100" />
			        <span class="textoListado">(100 caracteres m&aacute;ximo)</span></div>  
                    <div>
            ...
            <div >
		    	<label class="inputText" >Descipcion de Novedad:</label>
	    			<input name="descripcion_novedad" type="text" class="txtInput" id="descripcion_novedad"  value="" maxlength="70" />
			        <span class="textoListado">(70 caracteres m&aacute;ximo)</span></div>      
<div>
...
<div>
         ...
         <div>
		   ...
		   <input type="button" class="boton"  value="Cancelar" onclick="document.location.href='<? echo $this->modulo; ?>?<? $this->encodeURL('accion=listar&id_categoria='.$this->DATAFORM['formProducto']['id_categoria']); ?>';" />
	    </div>
		</div>
