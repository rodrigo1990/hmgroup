...
class CategoriaManager extends PFTableManager {
	...
	function insert($arrayData){
		/*INSERT*/
/*INSERT INTO ".$this->tabla." (id_categoria,nombre,id_padre,descripcion,activo,orden,id_usuario,folder) VALUES (NULL,'$arrayData[nombre]',$arrayData[id_padre],'$arrayData[descripcion]',$arrayData[activo],$arrayData[orden],$arrayData[id_usuario],'$arrayData[folder]') */
		if(!$this->validate($arrayData,'i')){
			return false;
		}
		$orden=$this->getNextOrden($arrayData);
		$sql="INSERT INTO ".$this->tabla." (id_categoria,nombre,id_padre,descripcion,activo,orden) VALUES (NULL,'$arrayData[nombre]',$arrayData[id_padre],'$arrayData[descripcion]',$arrayData[activo],$orden) ";
...
/*DELETE FROM ".$this->tabla." WHERE id_categoria=$id_categoria*/
	...
	function update($arrayData){
		/*UPDATE*/
/*UPDATE ".$this->tabla." SET nombre='$arrayData[nombre]' , id_padre=$arrayData[id_padre] , descripcion='$arrayData[descripcion]' , activo=$arrayData[activo] , orden=$arrayData[orden] , id_usuario=$arrayData[id_usuario] , folder='$arrayData[folder]' WHERE id_categoria=$arrayData[id_categoria]*/
		if(!$this->validate($arrayData,'u')){
			return false;
		}
		$sql="UPDATE ".$this->tabla." SET nombre='$arrayData[nombre]' , id_padre=$arrayData[id_padre] , descripcion='$arrayData[descripcion]' , activo=$arrayData[activo] WHERE id_categoria=$arrayData[id_categoria]";
		$query=$this->oDBM->query($sql);
		return $this->oDBM->affected_rows();
	}
...
FROM ".$this->tabla." ";
	...
	function getListaCategorias($idCat=0,$soloVisibles=false){
		...
		foreach($aCat as $ind => $item){
			if($this->tieneSubCats($item['id_categoria'])!=0){
				$aCat[$ind]['subcats']=$this->getListaCategorias($item['id_categoria'],$soloVisibles);
			}
		}
		return $aCat;
	}
	...
	function getOptionValues($idCat,$banned,$relleno){
		...
		foreach($aCat as $ind => $item){
			if($item['id_categoria']!=$banned){
				$this->cadena.='<option value="'.$item['id_categoria'].'">'.$relleno.$item['nombre'].'</option>';
			}
	...
	function actualizarCategoria($arrayData){
		...
		if(isset($arrayData['habilitar']) && $arrayData['id_padre']==0){
			$this->actualizarRelaciones($arrayData['id_categoria'],$arrayData['habilitar']);
		}
	...
	function activarCategoria($idCategoria){
		$sql="UPDATE ".$this->tabla." SET activo=1 WHERE id_categoria=$idCategoria";
		return $this->oDBM->query($sql);
	}
	
	function desactivarCategoria($idCategoria){
		$sql="UPDATE ".$this->tabla." SET activo=0 WHERE id_categoria=$idCategoria";
		return $this->oDBM->query($sql);
	}
	...
	function updateOrden($idCategoria,$orden){
		$sql="UPDATE $this->tabla SET orden=$orden WHERE id_categoria=$idCategoria";
		//echo $sql;
		$query=$this->oDBM->query($sql);
		return $this->oDBM->affected_rows();
	}
