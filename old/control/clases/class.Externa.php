...
class Externa extends PFApplication{
	...
	function getProductosEnPromocion($from=null,$qty=null){
		$oPro=new ProductoManager($this->oDBM);
		return $oPro->getPromociones($from,$qty);
	}
	
	function getCantProductosEnPromocion(){
		$oPro=new ProductoManager($this->oDBM);
		return $oPro->getCantPromociones();
	}
	...
	function getDatosNoticias(){
		
		$oNot=new NoticiaManager($this->oDBM);
		
		$listaNotas = $oNot->getData(null,null,array(" visible = 1")," orden ");
		
		$this->imagenes=array();
		$this->videos=array();
		
		foreach($listaNotas as $ind => $lista){
			$this->imagenes=$oNot->getImagenes($lista['id_noticia']);
			$this->videos=$oNot->getVideos(null,null,array(" id_relacion = ".$lista['id_noticia']));
			$listaNotas[$ind]['imagenes']=$this->imagenes;
			$listaNotas[$ind]['videos']=$this->videos;
		}
		return $listaNotas;  
	}
	...
	function getProductosCategoria($idCat,$from=null,$qty=null){
		$oPro=new ProductoManager($this->oDBM);
		return $oPro->getProductosCategoria($idCat,$from,$qty);
	}
	
	function getCantProductosCategoria($idCat){
		$oPro=new ProductoManager($this->oDBM);
		return $oPro->getCantProductosCategoria($idCat);
	}
