...
class Externa extends PFApplication{
	...
	function checkLoginUser($user,$pass){
		...
		if($oUsu->checkLoginUser($user,$pass)){
			return true;
		}
		else{
			$this->ERROR=true;
	...
	function downloadListaPrecios(){
		$oLista=new ListaManager($this->oDBM);
		$data=$oLista->getListaPrecios();
		if($data){
			PF::dowloadFile(PF::getPath(RUTA_CONTENIDO.'/'.$data['nombre']),'Lista_precios');
			exit();
		}
		$this->ERROR=true;
