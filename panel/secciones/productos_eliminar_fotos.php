		...
		function borrar_elemento(id)
		{
			(function($) 
			{
				jConfirm('Haz click en OK para borrarla.', 'Deseas borrar esta foto?', function(r) 
				{
					if(r == true)
					{
						$.getJSON("secciones/productos_eliminar_foto.php?elemento="+id, function(data){
						$('#tr_id_'+id).remove();
						});
					}
					else
	...
	$(document).ready(function(){ 
		...
		$(function() {
			$("#contentLeft ul").sortable({ opacity: 0.6, cursor: 'move', update: function() {
				...
				$.post("updateDB_productos_fotos.php", order, function(theResponse){
...
<section role="main" class="content-body">
	<header class="page-header">
		<h2>Productos - Eliminar Imágenes</h2>
	</header>
		...
		<div class="mg-main">
			<div class="row mg-files" data-sort-destination data-sort-id="media-gallery">
				<div id="contentLeft">
		        	<ul>
						<?php
						$query  = "SELECT id, foto, recordListingID  FROM productos_fotos ORDER BY recordListingID ASC";
						...
						while($fila = mysql_fetch_array($result, MYSQL_ASSOC))
						{
							...
							<li id="recordsArray_<?php echo $fila['id']; ?>">
								<div id="tr_id_<?php echo $fila['id']; ?>">
									<div style="position: absolute;"><img src="assets/images/cancel.png" title="Borrar" border="0" onclick="javascript:borrar_elemento('<?php echo $fila['id']; ?>');" style="cursor:pointer" /></div>
									<img src="productos/<?php echo $foto; ?>" style="width: 230px;" />
								</div>
							</li>
							<?php
						}
