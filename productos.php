...
$conn = new PDO("mysql:host=localhost;dbname=hmgroup_bd", 'root', '');
            ...
            if($_GET['c'])
            {
              ...
              if($categoria_seleccionada == $categoria_actual)
              {
                ...
                <?php
                 ...
                 $stmt = $conn->prepare("SELECT id, categoria, titulo FROM subcategorias WHERE categoria=:categoria_actual ORDER BY titulo ASC");
              $stmt->bindValue(':categoria_actual', $categoria_actual);

              $stmt->execute();
asdsad

                                          
                  while($fila_subcategoria =  $stmt->fetch(PDO::FETCH_ASSOC))
                  {
                    $titulo = $fila_subcategoria['titulo'];
